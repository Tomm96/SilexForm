...
class TraceableUrlMatcherTest extends \PHPUnit_Framework_TestCase
{
    ...
    {
        $routes = new RouteCollection();
        $routes->add('foo', new Route('/foo', array(), array(), array(), 'baz', array(), array(), "request.headers.get('UserData-Agent') matches '/firefox/i'"));
        ...
        $this->assertEquals("Condition \"request.headers.get('UserData-Agent') matches '/firefox/i'\" does not evaluate to \"true\"", $traces[0]['log']);
